  ...
  async execute(interaction: Interaction<CacheType>) {
    if (interaction.isChatInputCommand()) {
      ...
      if (interaction.customId === MODAL_ID_LOG_CUSTOM) {
        ...
        const amountStr = interaction.fields.getTextInputValue(
          INPUT_ID_CUSTOM_AMOUNT,
        );
        ...
        if (isNaN(amountMl) || amountMl <= 0) {
          await interaction.reply({
            content:
              "❌ Please enter a valid positive number for the amount in ml.",
            ephemeral: true,
          });
          return;
        }
        ...
        try {
          ...
          if (interaction.message) {
            try {
              ...
              if (originalMessage && originalMessage.components.length > 0) {
                const firstRow =
                  originalMessage.components[0].toJSON() as APIActionRowComponent<APIButtonComponent>;
                const disabledRow = new ActionRowBuilder<ButtonBuilder>();
                firstRow.components.forEach(
                  (componentJson: APIButtonComponent) => {
                ...
                console.log(
                  `[DEBUG] Disabled buttons on original message ${originalMessage.id} after modal submit`,
                );
              }
            } catch (editErr) {
              console.warn(
                "Could not edit original message to disable buttons after modal:",
                editErr,
              );
            }
          }
          ...
          const currentStreakData = await streakService.getStreakData(
            userId,
            STREAK_TYPES.WATER_DAILY_CONSISTENCY,
          );
          const streakSummaryMessage =
            streakFormatter.getStreakStatusMessage(currentStreakData);
...
${streakSummaryMessage}`,
                ephemeral: false,
              });
            } catch (followUpErr) {
              console.warn(
                "Could not follow up modal interaction for streak summary:",
                followUpErr,
              );
              try {
...
${streakSummaryMessage}`); // Fallback to DM
                ...
                console.error(
                  "Failed to send streak summary as DM after modal:",
                  dmErr,
                );
              }
            }
          ...
          if (interaction.replied || interaction.deferred) {
            await interaction.editReply({
              content: "❌ An error occurred while logging your custom amount.",
            });
          } else {
            await interaction.reply({
              content: "❌ An error occurred while logging your custom amount.",
